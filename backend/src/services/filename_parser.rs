...
mod tests {
    ...
    #[test]
    fn test_double_extension_strips_last_dot() {
        let (artist, title) = parse_filename("Artist - Title.backup.mp3");
        assert_eq!(artist.as_deref(), Some("Artist"));
        assert_eq!(title, "Title.backup");
    }
}
